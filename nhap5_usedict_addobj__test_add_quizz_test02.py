...
def getfull_slide_as_images(linkfile):
    newlinkfile=linkfile[:-5]+str('_cp.pptx')
    print(newlinkfile)
    new_title=newlinkfile
    shutil.copy(linkfile, newlinkfile)
    ...
    with ZipFile(template, "r") as zin:
        with ZipFile(newfile, "w") as zout:
            ...
            with zin.open("h5p.json", "r") as h5p:
                ...
                content["title"] = title
            zout.writestr("h5p.json", json.dumps(content))
def add_to_json_img_text_newver1(newfile, image_folder, images, title, arrayobj,arrobj_shape,images_media,folder_images_pptx_media,list_h5p_object_quest,numb):
    ...
    with ZipFile(template, "r") as zin:
        with ZipFile(newfile, "w") as zout:
            ...
            with zin.open("content/content.json") as fp:
                ...
                #add quest cho quiz
                count_slide=0
                for i, slide in enumerate(content["presentation"]["slides"]):
                    count_slide= count_slide + 1
                    print('Count slide ',i)
                x = 0
                for i, slide in enumerate(content["presentation"]["slides"]):
                    #print('values slide 0 element 1',slides[0]["elements"][1])
                    #print("quest_h5p_json:",quest_h5p_json)

                        # if i == j:
                        #     slide["elements"].append(list_h5p_object_quest[x])

                        # if i == count_slide - 2:
                        #     slide["elements"].append(list_h5p_object_quest[1])
                        # if i == count_slide - 3:
                        #     slide["elements"].append(list_h5p_object_quest[2])
...
def main():
    ...
    try:
        ...
        #lấy background trong file pptx_cp
        newlinkfilee=getfull_slide_as_images(filepath)
        ...
        if(1):
            ...
            #Get quest from text - > import to h5p
            if(False):
                aq, bq, cq = Get_text_to_addQuiz.load_filetext_from_folder(
                    r"C:\Users\ACER\anaconda3\envs\h5p\pptx2h5p_1_2_deep_copy_addimages-20230319T030947Z-001\pptx2h5p_1_2_deep_copy_addimages_NCKH")
            else:
                aq, bq, cq = Get_text_to_addQuiz.load_filetext_from_folder(folder)
            # for i in a:
            #     print(i)
            list_h5p_object_quest = []
            numb=[]
            for i, link in enumerate(bq):
                if "slide" in link:
                    print("slide ok")
                    numb = Get_text_to_addQuiz.read_text_from_file_text(link)
                    #print(Get_text_to_addQuiz.add_h5p_json_quest(az, "slide"))
                    #numb= Get_text_to_addQuiz.add_h5p_json_quest(az, "quizz")

                if "true_false" in link:
                    print("true_false ok")
                    az = Get_text_to_addQuiz.read_text_from_file_text(link)
                    # print("gia tri true false")
                    print(Get_text_to_addQuiz.add_h5p_json_quest(az, "true_false"))
                    list_h5p_object_quest.append(Get_text_to_addQuiz.add_h5p_json_quest(az, "true_false"))
                if "multil_choice" in link:
                    print("multil_choice ok")

                    az = Get_text_to_addQuiz.read_text_from_file_text(link)
                    print(Get_text_to_addQuiz.add_h5p_json_quest(az, "multil_choice"))
                    list_h5p_object_quest.append(Get_text_to_addQuiz.add_h5p_json_quest(az, "multil_choice"))
                if "quizz" in link:
                    print("quizz ok")
                    az = Get_text_to_addQuiz.read_text_from_file_text(link)
                    print(Get_text_to_addQuiz.add_h5p_json_quest(az, "quizz"))
                    list_h5p_object_quest.append(Get_text_to_addQuiz.add_h5p_json_quest(az, "quizz"))

            # add_to_json_img_text_newver1(
            #         newfilename, image_folder, images, title, arrayobj = listobj_text_slidee,arrobj_shape= list_shape_obj_ofslide,images_media=images_media,folder_images_pptx_media=image_media_folder,list_h5p_object_quest = list_h5p_object_quest
            #     )
            add_to_json_img_text_newver1(
                    newfilename, image_folder, images, title,arrayobj = [''] ,arrobj_shape= [''],images_media=images_media,folder_images_pptx_media=image_media_folder,list_h5p_object_quest = list_h5p_object_quest,numb=numb
            )
...
if __name__ == "__main__":
    main()
#27/12 chua viet xong phần code import to quiz. Viết tạm sang file khác