    ...
    with ZipFile(template, "r") as zin:
        with ZipFile(newfile, "w") as zout:
            ...
            with zin.open("h5p.json", "r") as h5p:
                ...
                content["title"] = title
            zout.writestr("h5p.json", json.dumps(content))
def add_to_json_img_text_newver1(newfile, image_folder, images, title, arrayobj,arrobj_shape,images_media,folder_images_pptx_media,list_h5p_object_quest):
    ...
    try:
        ...
        if(1):
            ...
            for i in bq:
                if i[-3:] == 'txt':
                    if i[0:9] == "true_false":
                        az = Get_text_to_addQuiz.read_text_from_file_text(i)

                        list_h5p_object_quest.append(Get_text_to_addQuiz.add_h5p_json_quest(az, i[0:9]))
                    if i[0:12] == "multil_choice":
                        az = Get_text_to_addQuiz.read_text_from_file_text(i)

                        list_h5p_object_quest.append(Get_text_to_addQuiz.add_h5p_json_quest(az, i[0:12]))
                    if i[0:12] == "multil_choice":
                        az = Get_text_to_addQuiz.read_text_from_file_text(i)
